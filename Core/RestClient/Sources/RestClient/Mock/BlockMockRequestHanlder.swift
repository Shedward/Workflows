...
public struct BlockMockRequestHanlder<RequestBody, ResponseBody>: MockRequestHandler
    ...
    private let responseBuilder: (RestRequest<RequestBody, ResponseBody>) -> ResponseBody

    init(
        requestFilter: RestRequestFilter<RequestBody, ResponseBody> = .any(),
        responseBuilder: @escaping (RestRequest<RequestBody, ResponseBody>) -> ResponseBody
    ) {
